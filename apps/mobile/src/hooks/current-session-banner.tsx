...
export function CurrentSessionBanner({ dbReady }: Props) {
  ...
  return (
    <View className="mx-4 mb-2 mt-1 rounded-2xl bg-slate-900/95 px-3 py-2 shadow-sm shadow-black/40">
      <View className="flex-row items-center justify-between">
        <View className="flex-row items-center">
          ...
          <View>
            ...
            <Text className="text-[10px] text-slate-400">#{shortId}</Text>
          </View>
        </View>

        <View className="items-end">
          <Text className="text-[10px] text-slate-400">Elapsed</Text>
          <Text className="font-mono text-[14px] font-semibold text-slate-50">
            {timerLabel}
          </Text>
        </View>
      </View>
